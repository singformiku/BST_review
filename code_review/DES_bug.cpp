...
viod setB(int &i , int in,int loc){
	int hl=0;
	hl=1<<(31-(loc-1));
	if (in)
		i=i|hl;
...
void pKEY(int &l,int &r){
	//lenght of array is 56
	int map[55]={57,49,41,33,25,17,9,1,58,50,42,34,26,18,10,2,59,51,43,35,27,19,11,3,60,52,44,36,63,55,47,39,31,23,15,7,62,54,46,38,30,22,14,6,61,53,45,37,29,21,13,5,28,20,12,4};
	int hl=l;
	int hr=r;
	for (int t=0,t<56,t++){
		if (map[t]>32){
...
int funcS(int num,int row,int column){\
	...
	return (S[(num-1)*64+row*16+coluwn]);
